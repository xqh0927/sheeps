...
import com.example.sheeps.data.R

/**
 * 统一错误文案映射：将 [ApiResult.Error.code] 映射为用户可见的 `com.example.sheeps.data.R.string.*` 资源 ID。
 *
 ...
 * - 其它    → 通用网络重试提示
 *
 * 注意：上述 5 个字符串已从 `:designsystem`(ui.R) 下沉至本模块 `:data`(data.R)，
 * 以断开 `:data` 对 `:designsystem` 的反向依赖（分层异味修复）。
 */
