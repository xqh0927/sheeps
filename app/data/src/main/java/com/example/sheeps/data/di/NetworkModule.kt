package com.example.sheeps.data.di

import com.apkfuns.logutils.LogUtils
import com.example.sheeps.data.BuildConfig
...
/**
 * 客户端网络配置 Hilt 依赖注入模块（置于 :data 模块中，实现数据与网络层自闭环）
 * 负责提供全局唯一的 Json 序列化解析器、配置核心拦截器的 OkHttpClient、以及 Retrofit 服务代理。
 */
@Module
...
object NetworkModule {
    ...
    fun provideOkHttpClient(prefs: UserPreferences, json: Json): OkHttpClient {
        ...
        // 针对网络异常或后端服务器偶发性 502/504 故障，在 1~2 秒内进行退避重试，最大尝试 3 次。
        // 写接口（POST）遇 5xx 直接返回、不重试，避免重复提交/重复发放道具；连接级 IOException 仍可重试 POST；GET 维持原逻辑。
        val weakNetworkRetryInterceptor = Interceptor { chain ->
            ...
            while (attempt <= 3) {
                ...
                if (attempt < 3) {
                    try {
                        val backoffDelay = (1L shl (attempt - 1)) * 1000L // 指数退避: 1s, 2s
