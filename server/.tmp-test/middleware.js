...
async function decryptRequest(request) {
    ...
    // 解密
    try {
        const decrypted = await (0, crypto_1.decryptAES)(bodyJson.encrypted);
        const newRequest = new Request(request.url, {
            method: request.method,
            headers: request.headers,
            body: decrypted,
 ...
 */
async function encryptResponse(response) {
    ...
    try {
        const encrypted = await (0, crypto_1.encryptAES)(bodyText);
