...
exports.getDifficultyForLevel = getDifficultyForLevel;
exports.lcg = lcg;
exports.generateSolvableLevel = generateSolvableLevel;
 ...
 */
function generateSealedClusters(nodes, rand, sealRatio, clusterCount, maxLayer) {
    if (nodes.length === 0)
        return new Map();
    const maxZ = Math.max(...nodes.map((n) => n.coord.z));
    let eligible = nodes.filter((n) => n.coord.z <= maxZ * 0.7);
    if (eligible.length < clusterCount)
        eligible = nodes;
    ...
    for (const seed of seeds) {
        ...
        while (queue.length > 0 && cluster.size < targetPerCluster) {
            ...
            const neighbors = nodes
                ...
                if (cluster.has(idx))
                    return false;
                if (n.coord.z !== currentNode.coord.z)
                    return false;
                if (Math.abs(n.coord.x - currentNode.coord.x) > 1.5)
                    return false;
                if (Math.abs(n.coord.y - currentNode.coord.y) > 1.5)
                    return false;
                return true;
            })
                .map(({ idx }) => idx);
...
function randomSealLayer(rand, maxLayer) {
    if (maxLayer === 1)
        return 1;
    if (maxLayer === 2)
        return rand() < 0.65 ? 1 : 2;
    const r = rand();
    if (r < 0.50)
        return 1;
    if (r < 0.85)
 ...
 * @return 关卡所有卡牌的数据列表
 */
function generateSolvableLevel(userId, levelId, seed) {
    let coordinates = [];
    // 第 1 关采用简单的新手引导固定布局
    if (levelId === 1) {
        coordinates = [
        ...
        const { cardCount: maxCards } = (0, difficulty_1.calculateCardCount)(userId, levelId);
        const possible = [];
    ...
    const nodes = coordinates.map((coord, index) => ({ index, coord, assignedType: -1 }));
