...
async function getGitHubAppUpdate(currentCode) {
    try {
...
exports.apkExistenceCache = new Map();
...
async function checkApkExists(url) {
    const now = Date.now();
    const cached = exports.apkExistenceCache.get(url);
    if (cached) {
    ...
    try {
        ...
        exports.apkExistenceCache.set(url, { exists, checkedAt: now });
        return exists;
    }
    catch {
        exports.apkExistenceCache.set(url, { exists: false, checkedAt: now });
        return false;
    }
}
async function getDatabaseAppUpdate(env, currentCode) {
    const versions = await env.DB.prepare('SELECT version_code, version_name, apk_url, update_log, is_force_update FROM app_version WHERE version_code > ? ORDER BY version_code DESC LIMIT 5').bind(currentCode).all();
...
const CACHE_STAGE_CONFIG = new Map();
...
function generateSolvableLevel(levelId, seed) {
    let coordinates = [];
    if (levelId === 1) {
        coordinates = [
...
async function handleWebSocketSession(socket, gameId, playerId, env) {
    const now = Date.now();
    // Send PLAYER_RECONNECTED system event to DB
    try {
        await env.DB.prepare('INSERT INTO game_commands (game_id, sender_id, command_data, created_at) VALUES (?, ?, ?, ?)').bind(gameId, 'SYSTEM', JSON.stringify({
            gameId,
            seqId: 0,
            timestamp: now,
            senderId: 'SYSTEM',
    ...
    let lastReadId = 0;
    try {
        const maxRow = await env.DB.prepare('SELECT MAX(id) as maxId FROM game_commands WHERE game_id = ?').bind(gameId).first();
    ...
    const intervalId = setInterval(async () => {
        ...
        try {
            const rows = await env.DB.prepare('SELECT id, command_data FROM game_commands WHERE game_id = ? AND id > ? AND sender_id != ? ORDER BY id ASC').bind(gameId, lastReadId, playerId).all();
    ...
    socket.addEventListener('message', async (event) => {
        try {
            ...
            await env.DB.prepare('INSERT INTO game_commands (game_id, sender_id, command_data, created_at) VALUES (?, ?, ?, ?)').bind(gameId, playerId, JSON.stringify(command), currentTime).run();
        }
        catch (e) {
    ...
    const handleDisconnect = async () => {
        ...
        setTimeout(async () => {
            try {
                const latestEvent = await env.DB.prepare('SELECT command_data FROM game_commands WHERE game_id = ? AND created_at >= ? ORDER BY id DESC LIMIT 1').bind(gameId, disconnectTime).first();
                if (latestEvent) {
                ...
                const matchEntry = await env.DB.prepare('SELECT player_id, matched_opponent FROM matchmaking_queue WHERE matched_game_id = ? LIMIT 1').bind(gameId).first();
                if (matchEntry) {
                    ...
                    if (!userExists) {
                        ...
                        await env.DB.prepare('INSERT OR IGNORE INTO users (id, phone, username, avatar, points, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(winnerId, null, defaultUsername, null, 0, Date.now()).run();
                    }
                    await env.DB.prepare('INSERT INTO leaderboard (user_id, level_id, score, clear_time_ms, achieved_at) VALUES (?, ?, ?, ?, ?)').bind(winnerId, 0, 9999, 99999, Date.now()).run();
                }
            }
            catch (err) {
...
exports.default = {
    async fetch(request, env) {
        ...
        if (path === '/api/match/join' && request.method === 'POST') {
            ...
            const opponent = await env.DB.prepare('SELECT player_id FROM matchmaking_queue WHERE player_id != ? AND matched_game_id IS NULL AND joined_at >= ? ORDER BY joined_at ASC LIMIT 1').bind(playerId, now - 30000).first();
            if (opponent) {
        ...
        if (path === '/api/match/status' && request.method === 'GET') {
            ...
            const entry = await env.DB.prepare('SELECT matched_game_id, matched_opponent, game_seed, duel_level, joined_at FROM matchmaking_queue WHERE player_id = ?').bind(playerId).first();
            if (!entry) {
        ...
        try {
            ...
            if (path === '/api/auth/login' && request.method === 'POST') {
                ...
                let user = await env.DB.prepare('SELECT id, phone, username, avatar, points FROM users WHERE phone = ?').bind(body.phone).first();
                let isNewUser = false;
                if (!user) {
                    isNewUser = true;
                    const uuid = crypto.randomUUID();
                    const defaultUsername = `国风玩家_${body.phone.slice(-4)}`;
                    await env.DB.prepare('INSERT INTO users (id, phone, username, avatar, points, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(uuid, body.phone, defaultUsername, null, 0, Date.now()).run();
                    // Unlock Level 1 by default
                    await env.DB.prepare('INSERT INTO level_unlock (user_id, level_id, unlocked_at) VALUES (?, 1, ?)').bind(uuid, Date.now()).run();
                    // Grant initial inventory items
                    const initialItems = ['UNDO', 'SHUFFLE', 'MOVEOUT', 'REVIVE'];
                    for (const item of initialItems) {
                        await env.DB.prepare('INSERT INTO user_items (user_id, item_type, count) VALUES (?, ?, ?)').bind(uuid, item, 1).run();
                    }
                ...
                if (body.device_uuid && body.device_uuid !== user.id) {
                    ...
                    if (guestUser) {
                        ...
                        // 2. Merge levels
                        const guestLevels = await env.DB.prepare('SELECT level_id FROM level_unlock WHERE user_id = ?').bind(body.device_uuid).all();
                        ...
                        // 3. Merge items
                        const guestItems = await env.DB.prepare('SELECT item_type, count FROM user_items WHERE user_id = ?').bind(body.device_uuid).all();
                        for (const row of guestItems.results) {
                            await env.DB.prepare('INSERT INTO user_items (user_id, item_type, count) VALUES (?, ?, ?) ' +
                                'ON CONFLICT(user_id, item_type) DO UPDATE SET count = count + ?').bind(user.id, row.item_type, row.count, row.count).run();
                        }
                ...
                const unlockedRows = await env.DB.prepare('SELECT level_id FROM level_unlock WHERE user_id = ?').bind(user.id).all();
                const unlockedLevels = unlockedRows.results.map(r => r.level_id);
                // Query inventory items
                const itemRows = await env.DB.prepare('SELECT item_type, count FROM user_items WHERE user_id = ?').bind(user.id).all();
                const items = itemRows.results;
                // Query sign in status
                const chinaToday = new Date(Date.now() + 8 * 3600000).toISOString().split('T')[0];
                const signToday = await env.DB.prepare('SELECT 1 FROM sign_record WHERE user_id = ? AND sign_date = ?').bind(user.id, chinaToday).first();
                const lastSign = await env.DB.prepare('SELECT streak FROM sign_record WHERE user_id = ? ORDER BY sign_date DESC LIMIT 1').bind(user.id).first();
                return new Response(JSON.stringify({
                    success: true,
                    token,
                    refreshToken,
            ...
            if (path === '/api/user/sync' && request.method === 'POST') {
                ...
                const oldUser = await env.DB.prepare('SELECT points FROM users WHERE id = ?').bind(authUser.userId).first();
                if (oldUser) {
                    ...
                    const backupPayload = JSON.stringify({
                        ...
                        unlocked_levels: oldLevels.results.map((r) => r.level_id),
                        items: oldItems.results
                    });
                    // Write to backup logs
                    await env.DB.prepare('INSERT INTO backup_save_log (user_id, save_data, created_at) VALUES (?, ?, ?)').bind(authUser.userId, backupPayload, Date.now()).run();
                    // Delete logs older than 7 days
                    await env.DB.prepare('DELETE FROM backup_save_log WHERE created_at < ?').bind(Date.now() - 7 * 86400000).run();
                }
                // Apply sync updates
                if (body.points !== undefined && body.points > 0) {
                    await env.DB.prepare('UPDATE users SET points = MAX(points, ?) WHERE id = ?').bind(body.points, authUser.userId).run();
                }
                if (body.unlocked_levels) {
                    for (const lvl of body.unlocked_levels) {
                        await env.DB.prepare('INSERT OR IGNORE INTO level_unlock (user_id, level_id, unlocked_at) VALUES (?, ?, ?)').bind(authUser.userId, lvl, Date.now()).run();
                    }
                }
                if (body.items) {
                    for (const item of body.items) {
                        await env.DB.prepare('INSERT OR REPLACE INTO user_items (user_id, item_type, count) VALUES (?, ?, ' +
                            'COALESCE((SELECT MAX(count, ?) FROM user_items WHERE user_id = ? AND item_type = ?), ?))').bind(authUser.userId, item.item_type, item.count, authUser.userId, item.item_type, item.count).run();
                    }
                }
                ...
                return new Response(JSON.stringify({
                    success: true,
                    user: updatedUser,
                    unlocked_levels: updatedLevels.results.map((r) => r.level_id),
            ...
            if (path === '/api/level/unlock' && request.method === 'POST') {
                ...
                const checkUnlock = await env.DB.prepare('SELECT 1 FROM level_unlock WHERE user_id = ? AND level_id = ?').bind(authUser.userId, body.level_id).first();
                if (checkUnlock) {
                    const userPoints = await env.DB.prepare('SELECT points FROM users WHERE id = ?').bind(authUser.userId).first();
                ...
                const configRow = await env.DB.prepare('SELECT value FROM config WHERE key = ?')
                    .bind(`level_${body.level_id}_unlock_points`).first();
                const cost = configRow ? parseInt(configRow.value, 10) : (body.level_id === 2 ? 50 : (body.level_id === 3 ? 100 : 200));
                const userRow = await env.DB.prepare('SELECT points FROM users WHERE id = ?').bind(authUser.userId).first();
            ...
            if (path === '/api/score/submit' && request.method === 'POST') {
                ...
                let userExists = await env.DB.prepare('SELECT points FROM users WHERE id = ?').bind(resolvedUserId).first();
                if (!userExists) {
                    const defaultUsername = resolvedUserId.startsWith('uuid-') ? `游客_${resolvedUserId.slice(-4)}` : `玩家_${resolvedUserId.slice(-4)}`;
                    await env.DB.prepare('INSERT INTO users (id, phone, username, avatar, points, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(resolvedUserId, null, defaultUsername, null, 0, Date.now()).run();
                    // Automatically unlock the first 3 levels for new guest/offline users so they can play them directly
                    for (const lvl of [1, 2, 3]) {
                        await env.DB.prepare('INSERT OR IGNORE INTO level_unlock (user_id, level_id, unlocked_at) VALUES (?, ?, ?)').bind(resolvedUserId, lvl, Date.now()).run();
                    }
                    userExists = { points: 0 };
                }
                // Check if first clear of this level
                const clears = await env.DB.prepare('SELECT COUNT(*) as count FROM leaderboard WHERE user_id = ? AND level_id = ?').bind(resolvedUserId, body.level_id).first();
                ...
                await env.DB.prepare('INSERT OR IGNORE INTO level_unlock (user_id, level_id, unlocked_at) VALUES (?, ?, ?)').bind(resolvedUserId, body.level_id + 1, Date.now()).run();
                // Increment daily task progress
                const chinaToday = new Date(Date.now() + 8 * 3600000).toISOString().split('T')[0];
                for (const tid of ['PLAY_3_GAMES', 'PLAY_5_GAMES']) {
                    const taskProgress = await env.DB.prepare('SELECT ut.progress, t.target_count, ut.is_completed FROM user_task ut JOIN task t ON ut.task_id = t.id WHERE ut.user_id = ? AND ut.task_id = ? AND ut.task_date = ?').bind(resolvedUserId, tid, chinaToday).first();
                    if (!taskProgress) {
                        const target = tid === 'PLAY_3_GAMES' ? 3 : 5;
                        await env.DB.prepare('INSERT INTO user_task (user_id, task_id, task_date, progress, is_completed, is_rewarded) VALUES (?, ?, ?, 1, 0, 0)').bind(resolvedUserId, tid, chinaToday).run();
                    }
                        ...
                        await env.DB.prepare('UPDATE user_task SET progress = ?, is_completed = ? WHERE user_id = ? AND task_id = ? AND task_date = ?').bind(newProg, completed, resolvedUserId, tid, chinaToday).run();
                    }
                }
                // Insert into leaderboard
                await env.DB.prepare('INSERT INTO leaderboard (user_id, level_id, score, clear_time_ms, achieved_at) VALUES (?, ?, ?, ?, ?)').bind(resolvedUserId, body.level_id, body.score, body.clear_time_ms, Date.now()).run();
            ...
            if (path === '/api/sign/today' && request.method === 'POST') {
                ...
                const chinaToday = new Date(Date.now() + 8 * 3600000).toISOString().split('T')[0];
                const checkSign = await env.DB.prepare('SELECT 1 FROM sign_record WHERE user_id = ? AND sign_date = ?').bind(authUser.userId, chinaToday).first();
                if (checkSign) {
                ...
                const lastSign = await env.DB.prepare('SELECT streak FROM sign_record WHERE user_id = ? AND sign_date = ?').bind(authUser.userId, chinaYesterday).first();
                const newStreak = lastSign ? lastSign.streak + 1 : 1;
                // Fetch reward config
                const rewardsConfigRow = await env.DB.prepare('SELECT value FROM config WHERE key = ?').bind('sign_rewards').first();
                ...
                await env.DB.prepare('INSERT OR REPLACE INTO user_task (user_id, task_id, task_date, progress, is_completed, is_rewarded) VALUES (?, ?, ?, ?, ?, ?)').bind(authUser.userId, 'SIGN_IN_ONCE', chinaToday, 1, 1, 0).run();
                return new Response(JSON.stringify({
                    success: true,
                    streak: newStreak,
            ...
            if (path === '/api/shop/exchange' && request.method === 'POST') {
                ...
                const shopItem = await env.DB.prepare('SELECT name, item_type, points_price, stock FROM shop_items WHERE id = ?').bind(body.shop_item_id).first();
                ...
                await env.DB.prepare('INSERT INTO user_items (user_id, item_type, count) VALUES (?, ?, ?) ' +
                    'ON CONFLICT(user_id, item_type) DO UPDATE SET count = count + ?').bind(authUser.userId, shopItem.item_type, body.count, body.count).run();
                await env.DB.prepare('INSERT INTO exchange_record (user_id, shop_item_id, item_type, count, points_cost, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(authUser.userId, body.shop_item_id, shopItem.item_type, body.count, totalCost, Date.now()).run();
                await env.DB.prepare('INSERT INTO point_record (user_id, type, amount, source, remaining_points, created_at) VALUES (?, ?, ?, ?, ?, ?)').bind(authUser.userId, 'OUT', totalCost, `SHOP_REDEEM_${shopItem.item_type}`, remainingPoints, Date.now()).run();
                const backpackItem = await env.DB.prepare('SELECT count FROM user_items WHERE user_id = ? AND item_type = ?')
                    .bind(authUser.userId, shopItem.item_type).first();
                return new Response(JSON.stringify({
                    success: true,
            ...
            if (path === '/api/task/daily' && request.method === 'GET') {
                ...
                for (const task of allTasks.results) {
                    let userTask = await env.DB.prepare('SELECT progress, is_completed, is_rewarded FROM user_task WHERE user_id = ? AND task_id = ? AND task_date = ?').bind(authUser.userId, task.id, chinaToday).first();
                    if (!userTask) {
                        await env.DB.prepare('INSERT INTO user_task (user_id, task_id, task_date, progress, is_completed, is_rewarded) VALUES (?, ?, ?, 0, 0, 0)').bind(authUser.userId, task.id, chinaToday).run();
            ...
            if (path === '/api/task/claim' && request.method === 'POST') {
                ...
                const chinaToday = new Date(Date.now() + 8 * 3600000).toISOString().split('T')[0];
                const userTask = await env.DB.prepare('SELECT is_completed, is_rewarded FROM user_task WHERE user_id = ? AND task_id = ? AND task_date = ?').bind(authUser.userId, body.task_id, chinaToday).first();
                ...
                const taskRow = await env.DB.prepare('SELECT points_reward FROM task WHERE id = ?').bind(body.task_id).first();
            ...
            if (path === '/api/leaderboard' && request.method === 'GET') {
                ...
                const results = await env.DB.prepare(`SELECT u.username, u.avatar, l.clear_time_ms, l.score, l.achieved_at 
           FROM leaderboard l 
           JOIN users u ON l.user_id = u.id 
           WHERE l.level_id = ? AND l.achieved_at >= ?
           ORDER BY l.clear_time_ms ASC 
           LIMIT ? OFFSET ?`).bind(levelId, timeFilter, limit, offset).all();
            ...
            if (path === '/api/user/profile' && request.method === 'GET') {
                ...
                const lastSign = await env.DB.prepare('SELECT streak FROM sign_record WHERE user_id = ? ORDER BY sign_date DESC LIMIT 1').bind(authUser.userId).first();
                const highestCleared = await env.DB.prepare('SELECT MAX(level_id) as highest FROM leaderboard WHERE user_id = ?').bind(authUser.userId).first();
                return new Response(JSON.stringify({
                    success: true,
                    user,
                    unlocked_levels: levels.results.map((r) => r.level_id),
                    items: items.results,
            ...
            if (path === '/api/user/exchange-history' && request.method === 'GET') {
                ...
                const records = await env.DB.prepare('SELECT shop_item_id, item_type, count, points_cost, created_at FROM exchange_record WHERE user_id = ? ORDER BY created_at DESC LIMIT 50').bind(authUser.userId).all();
                return new Response(JSON.stringify(records.results), { headers: corsHeaders });
            }
