...
exports.handleMatchRoutes = handleMatchRoutes;
...
async function handleMatchRoutes(request, env, path, url) {
    ...
    if (path === '/api/match/status' && request.method === 'GET') {
        ...
        if (entry.matched_game_id)
            return new Response(JSON.stringify({ status: 'matched', gameId: entry.matched_game_id, opponentId: entry.matched_opponent, duelLevel: entry.duel_level, gameSeed: entry.game_seed }), { headers: corsHeaders });
