...
exports.handleTaskRoutes = handleTaskRoutes;
...
async function handleTaskRoutes(request, env, path, lang) {
    ...
    if (path === '/api/task/daily' && request.method === 'GET') {
        ...
        const taskList = allTasks.results;
        const userTaskList = userTasks.results;
        const inserts = [], list = [];
    ...
    if (path === '/api/task/claim' && request.method === 'POST') {
        ...
        const userTask = userTaskResult.results[0];
        ...
        const reward = taskResult.results[0]?.points_reward || 10;
        const remainingPoints = (userResult.results[0]?.points || 0) + reward;
