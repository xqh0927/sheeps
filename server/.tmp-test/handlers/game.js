...
exports.handleGameRoutes = handleGameRoutes;
...
async function handleGameRoutes(request, env, path, url) {
    ...
    if (path === '/api/level' && request.method === 'GET') {
        ...
        // 调用确定性必定可解的关卡生成算法
        const newLayout = (0, level_1.generateSolvableLevel)(userId, levelId, seed);
    ...
    if (path === '/api/level/unlock' && request.method === 'POST') {
        ...
        const costStr = await (0, helpers_1.getCachedConfig)(env, configKey, body.level_id === 2 ? '50' : '200');
        ...
        if (unlockCheckResult.results.length > 0)
    ...
    if (path === '/api/score/submit' && request.method === 'POST') {
        ...
        const expectedSign = await (0, crypto_1.sha256)(`${body.user_id}_${body.level_id}_${body.clear_time_ms}_folklore`);
        ...
        let userExists = userResult.results[0];
        const mutations = [];
    ...
    if (path === '/api/sign/today' && request.method === 'POST') {
        ...
        // 从 KV 缓存读取签到奖励配置
        const rewardsStr = await (0, helpers_1.getCachedConfig)(env, 'sign_rewards', '20,20,30,30,40,50,100');
    ...
    if (path === '/api/leaderboard' && request.method === 'GET') {
        ...
        if (type === 'daily')
            ...
            timeFilter = todayStart - ((new Date(now + 8 * 3600000).getDay() + 6) % 7) * 24 * 3600000;
        // 执行 SQL：按分数从高到低、通关耗时从短到长进行中国特色消消乐排行榜排序（限制每个用户只取个人的最高记录，防刷榜）
        const results = await env.DB.prepare(`SELECT username, avatar, clear_time_ms, score, achieved_at FROM (SELECT u.username, u.avatar, l.clear_time_ms, l.score, l.achieved_at, ROW_NUMBER() OVER (PARTITION BY l.user_id ORDER BY l.score DESC, l.clear_time_ms ASC) as rn FROM leaderboard l JOIN users u ON l.user_id = u.id WHERE l.level_id = ? AND l.achieved_at >= ?) WHERE rn = 1 ORDER BY score DESC, clear_time_ms ASC LIMIT ? OFFSET ?`).bind(levelId, timeFilter, limit, offset).all();
    ...
    if (path === '/api/leaderboard/daily-popup' && request.method === 'GET') {
        ...
        let yesterdayRank = 0;
        if (authUser1) {
            const userTotal = await env.DB.prepare('SELECT SUM(score) as points FROM leaderboard WHERE user_id = ? AND achieved_at >= ? AND achieved_at <= ?').bind(authUser1.userId, yesterdayStart1, yesterdayEnd1).first();
            const points = userTotal?.points ?? 0;
            if (points > 0) {
                const higher = await env.DB.prepare('SELECT COUNT(*) as cnt FROM (SELECT user_id, SUM(score) as points FROM leaderboard WHERE achieved_at >= ? AND achieved_at <= ? GROUP BY user_id HAVING points > ?)').bind(yesterdayStart1, yesterdayEnd1, points).first();
