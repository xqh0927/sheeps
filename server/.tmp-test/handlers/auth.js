...
async function handleAuthRoutes(request, env, path) {
    ...
    if (path === '/api/auth/send-code' && request.method === 'POST') {
        ...
        if (!body.phone)
            return new Response(JSON.stringify({ error: '请填写手机号' }), { status: 400, headers: corsHeaders });
        // 生成 6 位随机数字验证码
    ...
    if (path === '/api/auth/login' && request.method === 'POST') {
        ...
        if (!body.phone || !body.code)
            return new Response(JSON.stringify({ error: '请填写手机号和验证码' }), { status: 400, headers: corsHeaders });
        ...
        const tokenRecord = codeRecord.results[0];
        ...
        let user = userRow.results[0];
        // 准备响应数据（默认使用预查询结果）
        let finalLevels = unlockedResult.results.map((r) => r.level_id);
        let finalItems = itemsResult.results;
        let finalSigned = signTodayRow.results[0] !== null && signTodayRow.results[0] !== undefined;
        let finalStreak = lastSignResult.results[0]?.streak || 0;
        ...
        if (body.device_uuid && body.device_uuid !== user.id) {
            ...
            if (guestUser) {
                ...
                for (const row of guestLevelsResult.results) {
                    ...
                    if (!finalLevels.includes(row.level_id))
                        finalLevels.push(row.level_id);
                }
                // 道具叠加更新：合并游客道具到响应数据
                for (const row of guestItemsResult.results) {
                    ...
                    const existing = finalItems.find((i) => i.item_type === row.item_type);
                    if (existing)
                        existing.count += row.count;
                    else
                        finalItems.push({ item_type: row.item_type, count: row.count });
                }
    ...
    if (path === '/api/auth/refresh' && request.method === 'POST') {
        ...
        if (!body.refreshToken)
            return new Response(JSON.stringify({ error: '缺少刷新令牌' }), { status: 400, headers: corsHeaders });
        // 验证 Refresh Token 合法性与过期时间
        const payload = await (0, crypto_1.verifyJWT)(body.refreshToken);
        if (!payload || payload.type !== 'refresh')
            return new Response(JSON.stringify({ error: '登录凭证已过期，请重新登录' }), { status: 401, headers: corsHeaders });
        // 重新签发一组新的双 Token
    ...
    if (path === '/api/auth/register' && request.method === 'POST') {
        ...
        env.DB.prepare('DELETE FROM login_token WHERE phone = ?').bind(body.phone).run().catch(() => { });
        // 哈希密码
        const pwHash = await (0, auth_utils_1.hashPassword)(body.password);
        const chinaToday = new Date(Date.now() + 8 * 3600000).toISOString().split('T')[0];
        const uuid = crypto.randomUUID();
        const defaultUsername = `国风玩家_${body.phone.slice(-4)}`;
        const insertQueries = [
        ...
        const token = await (0, crypto_1.generateJWT)({ userId: uuid, phone: body.phone, type: 'access', exp: Date.now() + 7200000 });
        const refreshToken = await (0, crypto_1.generateJWT)({ userId: uuid, phone: body.phone, type: 'refresh', exp: Date.now() + 30 * 86400000 });
        return new Response(JSON.stringify({
            success: true, token, refreshToken, user,
            unlocked_levels: [1],
    ...
    if (path === '/api/auth/login-password' && request.method === 'POST') {
        ...
        const pwValid = await (0, auth_utils_1.verifyPassword)(body.password, user.password_hash);
        if (!pwValid)
    ...
    if (path === '/api/auth/reset-password' && request.method === 'POST') {
        ...
        // 清除验证码
        env.DB.prepare('DELETE FROM login_token WHERE phone = ?').bind(body.phone).run().catch(() => { });
        const pwHash = await (0, auth_utils_1.hashPassword)(body.newPassword);
