 ...
 * 难度曲线模块（服务端）
 * ======================
 * 业务位置：在关卡生成（`level.ts`）及商城/任务/游戏等接口中，根据 (userId, levelId)
 * 计算「千人千面」的难度系数与卡牌数量，是关卡内容规模的核心杠杆。
 *
 * 难度杠杆设计：
 *  - 前 3 关固定难度；第 4 关起进入 1~100 难度段，每个难度段覆盖 1~2 关，
 *    由 `lcg(userId*9973 + D*10007)` 确定性决定，保证同一用户进度可复现。
 *  - 卡牌数量随难度单调上升（D≥4 使用幂律曲线 `60 + 240*((D-3)/97)^0.55`），
 *    并向下取整到 3 的倍数（消除规则为三连，须为 3 的整数倍）。
 *  - 每 5 关为休息关（卡牌数打八折），降低长线疲劳。
 *  - `getBaseCardCount` 仅算基础数量；`calculateCardCount` 再叠加休息关折扣与 [12,300] 安全边界。
 *
 * 并发/缓存说明：`difficultyCache` 为模块级单例 Map，仅在单个 Worker isolate 内命中，
 * 跨 isolate 不共享；其仅作纯计算结果的轻量缓存，无外部副作用。
 */
