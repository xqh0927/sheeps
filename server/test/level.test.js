...
test('同一个关卡传入不同随机种子时，生成的卡牌数量完全相同，但卡牌坐标与花色随机变化', () => {
  const levelId = 10;
  ...
  // 2. 坐标排布在不同 seed 时应该不同（布局变化）
  const coordsMatch = tilesSeed1.every((t, i) => t.x === tilesSeed2[i].x && t.y === tilesSeed2[i].y && t.z === tilesSeed2[i].z);
  assert.ok(!coordsMatch, '坐标排布在不同 seed 时应该变化');
